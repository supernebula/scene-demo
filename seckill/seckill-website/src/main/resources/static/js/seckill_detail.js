...
var seckill = {
    ...
    //处理秒杀逻辑
    handleSeckill: function(seckillId, node, payAmount){
        //获取秒杀地址，控制显示逻辑，执行秒杀
            ...
            if (result && result['success']){
                ...
                if (exposer['exposed']){
                    ...
                    $('#killBtn').one('click', function(){
                        ...
                        //2. 发送秒杀请求，执行秒杀
                        $.post(killUrl, {payAmount: payAmount}, function(result){
                            if (result && result['success']){
    ...
    //计时
    countdown: function (seckillId, nowTime, startTime, endTime, payAmount) {
        ...
        if (nowTime > endTime){
            ...
            seckillTimeSpan.countdown(killTime, function(event){
                ...
                //获取秒杀地址，控制实现逻辑，执行秒杀
                seckill.handleSeckill(seckillId, seckillBox, payAmount);
            });
        }else{
            //秒杀开始
            seckill.handleSeckill(seckillId, seckillBox, payAmount);
        ...
        init: function (params) {
            ...
            $.get(seckill.URL.now(), {}, function (result) {
                if (result && result['success']) {
                    ...
                    //时间判断
                    seckill.countdown(seckillId, nowTime, startTime, endTime, payAmount);
                } else {
                    console.log('result:' + result);
                }
            });
